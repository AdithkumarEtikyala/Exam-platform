...
import { Clock, HelpCircle, FileText, CheckCircle } from 'lucide-react';
...
export function StudentExamList() {
  ...
  const { data: submissions, isLoading: isLoadingSubmissions } = useCollection<StudentExam>(studentExamsQuery);

  const completedExams = useMemo(() => {
    if (!submissions) return new Set();
    return new Set(submissions.map(s => s.examId));
  }, [submissions]);
  ...
  return (
    <div className="space-y-4">
      {exams.map(exam => {
        ...
        return (
          <Card key={exam.id} className={isCompleted ? 'bg-muted/50' : ''}>
            <CardHeader>
